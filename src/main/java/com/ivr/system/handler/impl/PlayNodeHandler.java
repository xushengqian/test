...
public class PlayNodeHandler implements NodeHandler {
    ...
    public IvrNode.NodeType getNodeType() {
        return IvrNode.NodeType.PLAY;
    }

    @Override
    public IvrResult execute(CallSession session, IvrNode node, IvrFlow flow) {
        log.info("Executing play node [{}] for session [{}]", node.getId(), session.getSessionId());
        ...
        try {
            ...
            log.error("Error executing play node: {}", e.getMessage(), e);
